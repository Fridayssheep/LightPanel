...
主启动方式是 Docker Compose。部署时会启动两个容器：`ops-agent-backend` 运行 FastAPI 后端并挂载宿主机 Docker socket，`ops-agent-web` 使用 Nginx 托管前端构建产物并反代后端 API：
...
默认只扫描后端容器内 `/app/samples` 下的 Compose 文件，并允许 WebUI 编辑这些 Compose 文件。如果要像面板工具那样发现自己的项目，把宿主机项目目录挂到后端容器内，并加入 `OPS_AGENT_PROJECT_ROOTS`。需要 WebUI 编辑时不要使用只读挂载：

```yaml
environment:
  OPS_AGENT_PROJECT_ROOTS: |
    /app/samples
    /projects
volumes:
  - /你的/项目目录:/projects
...
支持识别 `compose.yml`、`compose.yaml`、`docker-compose.yml`、`docker-compose.yaml`。扫描只会发生在 `OPS_AGENT_PROJECT_ROOTS` 配置的目录内，不会全盘搜索。多个目录推荐在设置页一行一条填写；环境变量仍兼容英文逗号分隔。
...
```bash
OPS_AGENT_EXTERNAL_MCP_SERVERS='{"web":{"command":"uvx","args":["--from","mcp-server-fetch","mcp-server-fetch"],"description":"读取网页教程"}}'
```
...
```bash
OPS_AGENT_ENABLE_PUBLIC_MCP=true
```
...
默认 CUI 会把最近 30 轮用户/助手对话作为短期上下文发送给后端，所以可以连续问“它的日志”“重启它”这类省略表达。可以用参数关闭或调整：

```bash
ops-agent-cli --history-turns 0
ops-agent-cli --history-turns 20
```
...
```text
查看当前 Docker 容器状态
帮我部署一个 Nginx，映射到 8080 端口
重启 ops-agent-nginx 容器
停止 ops-agent-nginx 容器
分析日志：Address already in use, nginx bind failed
```
...
curl -X POST http://127.0.0.1:8000/agent/chat \
  -H "Content-Type: application/json" \
  -d '{"message":"停止 ops-agent-nginx 容器"}'
```
...
curl -X POST http://127.0.0.1:8000/agent/chat \
  -H "Content-Type: application/json" \
  -d '{"message":"停止 ops-agent-nginx 容器","approve_actions":true}'
```
